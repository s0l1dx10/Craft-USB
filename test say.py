...
IO_LEDS = {"red":19,"green":11,"blue":13} # 
...
# elif color == "orange":
#   rgb = (255, 87, 51)
# # set the RGB LED to the desired color
# pwm_red = PWM(Pin(IO_LEDS["red"]))
# pwm_red.duty(rgb[0])
# pwm_green = PWM(Pin(IO_LEDS["green"]))
# pwm_green.duty(rgb[1])
# pwm_blue = PWM(Pin(IO_LEDS["blue"]))
# pwm_blue.duty(rgb[2])
...
# # Define the GPIO pins connected to the dip switches
# IO_DIP = [2, 3, 4, 17]

# # Set up the GPIO pins for the dip switches
# for dipPin in IO_DIP:
#     GPIO.setup(dipPin, GPIO.IN, pull_up_down=GPIO.PUD_UP)

# # Read the state of the dip switches in a loop
# while True:
#     dipStates = []
#     for dipPin in IO_DIP:
#         if GPIO.input(dipPin) == GPIO.LOW:
#             dipStates.append(1)
